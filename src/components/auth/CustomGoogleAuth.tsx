'use client';

import React, { useState } from 'react';
import { Button } from '@/components/ui';
import styles from './CustomGoogleAuth.module.scss';
import { signInWithGoogle } from '@/lib/auth';

interface CustomGoogleAuthProps {
  onSuccess: (userData: any) => void;
  onError: (error: string) => void;
  isLoading?: boolean;
  buttonText?: string;
}

const CustomGoogleAuth: React.FC<CustomGoogleAuthProps> = ({
  onSuccess,
  onError,
  isLoading = false,
  buttonText = 'Continue with Google'
}) => {
  const [isAuthenticating, setIsAuthenticating] = useState(false);

  const handleGoogleAuth = async () => {
    setIsAuthenticating(true);
    
    try {
      // Create a smaller, centered popup window
      const popup = window.open(
        'about:blank',
        'google-auth',
        'width=500,height=600,scrollbars=yes,resizable=yes,status=yes,location=yes,toolbar=no,menubar=no,left=' + 
        (window.screen.width / 2 - 250) + ',top=' + (window.screen.height / 2 - 300)
      );

      if (!popup) {
        throw new Error('Popup was blocked by your browser. Please allow popups for this site.');
      }

      // Show custom loading content in popup with your branding
      popup.document.write(`
        <!DOCTYPE html>
        <html>
        <head>
          <title>Sign in with Google</title>
          <style>
            body {
              font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
              margin: 0;
              padding: 40px 20px;
              background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
              color: white;
              text-align: center;
              min-height: 100vh;
              display: flex;
              flex-direction: column;
              justify-content: center;
              align-items: center;
            }
            .logo {
              width: 60px;
              height: 60px;
              background: white;
              border-radius: 12px;
              margin: 0 auto 20px;
              display: flex;
              align-items: center;
              justify-content: center;
              font-size: 24px;
              font-weight: bold;
              color: #667eea;
            }
            .spinner {
              width: 40px;
              height: 40px;
              border: 3px solid rgba(255,255,255,0.3);
              border-top: 3px solid white;
              border-radius: 50%;
              animation: spin 1s linear infinite;
              margin: 20px auto;
            }
            @keyframes spin {
              0% { transform: rotate(0deg); }
              100% { transform: rotate(360deg); }
            }
            h2 {
              margin: 0 0 10px;
              font-size: 24px;
              font-weight: 600;
            }
            p {
              margin: 0;
              opacity: 0.9;
              font-size: 16px;
            }
          </style>
        </head>
        <body>
          <div class="logo">K</div>
          <h2>Connecting to Google</h2>
          <p>Please wait while we securely connect your account...</p>
          <div class="spinner"></div>
        </body>
        </html>
      `);

      // Perform the actual Google authentication
      const result = await signInWithGoogle();
      
      // Close popup and handle success
      popup.close();
      onSuccess(result.user);
      
    } catch (error: any) {
      console.error('Custom Google auth error:', error);
      
      // Handle specific error cases
      if (error.code === 'auth/popup-closed-by-user') {
        // User closed popup - don't show error
        console.log('User cancelled Google sign-in');
      } else if (error.code === 'auth/popup-blocked') {
        onError('Popup was blocked by your browser. Please allow popups for this site and try again.');
      } else {
        onError(error.message || 'Failed to sign in with Google');
      }
    } finally {
      setIsAuthenticating(false);
    }
  };

  return (
    <Button
      variant="outline"
      onClick={handleGoogleAuth}
      disabled={isLoading || isAuthenticating}
      fullWidth
      className={styles.googleButton}
    >
      <div className={styles.buttonContent}>
        <svg className={styles.googleIcon} viewBox="0 0 24 24">
          <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
          <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
          <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
          <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
        </svg>
        <span>
          {isAuthenticating ? 'Connecting...' : buttonText}
        </span>
      </div>
    </Button>
  );
};

export default CustomGoogleAuth;
