...
import { signInWithGoogle, sendVerificationEmail, getUserProfile, createUserProfile } from '@/lib/auth';
...
const AuthForm: React.FC<AuthFormProps> = ({
  ...
  const [pendingEmailAuth, setPendingEmailAuth] = useState<{email: string, type: 'signin' | 'signup', userData?: any} | null>(null);
  ...
  // Handle email sign in - now triggers email verification
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    if (validateSignInForm()) {
      ...
      try {
        // Send verification email and go directly to verification step
        await sendVerificationEmail(signInForm.email);
        setPendingEmailAuth({
  ...
  // Handle email sign up - now triggers email verification
  ...
  const handleEmailVerificationSuccess = () => {
    ...
    // The useEffect will handle navigation when user is authenticated
  };

  // Handle back from email verification
